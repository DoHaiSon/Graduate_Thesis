...
%\begin{itemize}
%	\item[$\ast$] \textbf{Hiệu chỉnh $\textbf{time sampling}$}
%\end{itemize} 

%Việc đồng bộ thời gian lấy mẫu giữa các SDR, thường chỉ cần thực hiện với các hệ thông không chia sẻ chung xung đồng hồ hệ thống với nhau, hay sử dụng thêm bất kỳ thiết bị phần cứng nào. Do đã chia sẻ xung đồ hồ chủ như ở phía trên, nên bước này sẽ được bỏ qua.

\begin{itemize}
	\item[$\ast$] \textbf{$\textbf{Hiệu chỉnh } \textbf{phase}_\textbf{offset}$}
\end{itemize} 

%/TODO tính toán sai khác pha liên tục để đưa ra sự sai khác về tần số thu của các SDR. Tuy nhiên điều này chưa được thực hiện do việc chia xung đồng hồ tham chiếu đã giúp đồng bộ điiều này.
...
%Cách để tính pha của tín hiệu lấy mẫu IQ là:
%Giả sử tín hiệu cơ sở $s(k)$ với tần số sóng mang offset $f_{0}$ (hoặc $\omega_{0}$), có biểu diễn toán học:
%\begin{equation}
...
%Do đó, phục hồi pha của tín hiệu bản chất cũng là phục hồi lại tần số của tín hiệu, đây là phương pháp phổ biến để ước lượng $f_{\textrm{offset}}$ do nó không thể được đo đạc trực tiếp như pha. Cách để tính pha của tín hiệu lấy mẫu IQ là:
%\begin{equation}
%	\theta = tan^{-1} \left (\frac{Im (x(k))}{Re(x(k))} \right )
%\end{equation}
%với $Re$ và $Im$ lần lượt là phần thực và ảo của mẫu IQ. Tương tự với các bộ thu khác, độ lệch pha giữa các SDR sẽ được tính ra liên tục, có thể được sử dụng làm thông số để cân chỉnh hệ SDR. Tuy nhiên trên thực tế, khi áp dụng SDR phương pháp đơn giản này chỉ thích hợp khi các tín hiệu đầu vào là tín hiệu từ máy phát chuẩn, qua bộ chia có bù pha đến tất cả các SDR mảng thu như trong dự án \cite{TravisFCollins} sử dụng USRP với tín hiệu sine chuẩn để đồng bộ pha.
%\begin{equation}
%\begin{split}
%	\Delta \theta &= \theta_1 - \theta_0 \\
%	\hat{\mathbf{x}}_1 &= \mathbf{x}_1 \times  e^{j \Delta \theta}
%\end{split}
%\end{equation}
