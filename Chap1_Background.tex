...
%với $\mathbf{E} = [\mathbf{e}_{1}, ..., \mathbf{e}_{M}]$, $\mathbf{E}_{\mathbf{s}} = [\mathbf{e}_{1}, ..., \mathbf{e}_{D}]$, $\mathbf{E}_{\mathbf{n}} = [\mathbf{e}_{D+1}, ..., \mathbf{e}_{M}]$, $\Lambda = diag\{\lambda_{1}, ..., \lambda_{M}\}$, $\Lambda_{\mathbf{s}} = diag\{\lambda_{1}, ..., \lambda_{D}\}$, $\Lambda_{\mathbf{n}} = diag\{\lambda_{D}, ..., \lambda_{M}\}$ và $\tilde{\Lambda}_{\mathbf{s}} = \Lambda_{\mathbf{s}} - {\mathbf{\sigma}_{\mathbf{n}}}^{2}\mathbf{I}_{\mathrm{n}}$. 
...
%Sắp xếp các giá trị riêng ($\Lambda = diag\{\lambda_{1} > ... > \lambda_{M}\}$) theo thứ tự giảm dần $(\lambda_{1} > ... > \lambda_{M})$ thì ta có thể tách được ra hai ma trận: ma trận chứa D vector riêng tương ứng với không gian con tín hiệu (Signal Subspace) là $\mathbf{E}_{\mathbf{s}} \triangleq [\mathbf{e}_{1}, ..., \mathbf{e}_{D}]$; ma trận chứa M - D vector riêng tương ứng với không gian con của tạp âm (Noise Subspace) là $\mathbf{E}_{\mathbf{n}} \triangleq [\mathbf{e}_{D+1}, ..., \mathbf{e}_{M}]$. Để phân tích chi tiết các thuộc tính cấu trúc riêng của ma trận hiệp phương sai $\mathbf{R}_{\mathbf{x}}$ tham khảo tại \cite{Schmidt2009a}.
